...
export async function handleAIResponse(
  ...
  try {
    ...
    if (aiAssistantDoc.exists()) {
      const data = aiAssistantDoc.data();
      if (data.behavior) aiSettings.behavior = data.behavior;
      if (data.template) aiSettings.template = data.template;
    }
  } catch (e) {
    console.error('Error fetching ai_assistant settings:', e);
  }
...
`;
    ...
    const systemInstruction = `Você é o assistente virtual da Oficina. Siga estritamente o fluxo de orçamento: 1) Ofereça o orçamento. 2) Se aceito, peça o nome. 3) Só então use generateQuote. Se o cliente aprovar um orçamento pendente, use acceptQuote.
Diretrizes de Comportamento Adicionais:
${aiSettings.behavior || ''}
