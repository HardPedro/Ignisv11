...
export function Home() {
  ...
  useEffect(() => {
    const fixData = async () => {
      if (userData?.role === 'Gestor' || userData?.role === 'SuperAdmin') {
        if (!userData.tenantId) return;
        try {
          // Fix whatsapp_numbers
          const numbersRef = collection(db, 'whatsapp_numbers');
          const qNumbers = query(numbersRef, where('tenantId', '==', userData.tenantId));
          const numbersSnap = await getDocs(qNumbers);
          numbersSnap.forEach(async (docSnap) => {
            if (!docSnap.data().server_token) {
              await updateDoc(doc(db, 'whatsapp_numbers', docSnap.id), {
          ...
          const qConvs = query(convsRef, where('tenantId', '==', userData.tenantId));
          const convsSnap = await getDocs(qConvs);
          convsSnap.forEach(async (docSnap) => {
            if (!docSnap.data().server_token) {
              await updateDoc(doc(db, 'whatsapp_conversations', docSnap.id), {
                server_token: 'ignishard18458416'
              });
            }
          });
